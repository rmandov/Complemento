...
const radius = ref(5000) // metros por defecto
const { center, register: registerClick, unregister: unregisterClick } = useMapInteractions(map)
const { filteredFeatures } = useProximitySearch(
  map,
  center,
  radius,
  pointsProyectos.features, // ← shallowRef con las features
  capaProyectos, // ← capa de CircleMarkers para resaltar
)
...
async function crearCapaProyectos() {
  if (!datosProyectos) {
    ...
    console.log('Base ligera: ', datosProyectos.features)
